...
class SupplierForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['phone'].widget.attrs.update({'class': 'textinput form-control', 'maxlength': '11', 'pattern' : '[0-9]{11}', 'title' : 'Numbers only'})
        self.fields['email'].widget.attrs.update({'class': 'textinput form-control'})
        self.fields['gstin'].widget.attrs.update({'class': 'textinput form-control', 'maxlength': '15', 'pattern' : '[0-9]{3}-[0-9]{3}-[0-9]{3}-[0-9]{3}', 'title' :'Tax Identification Number (TIN) Format: XXX-XXX-XXX-XXX', 'placeholder': 'xxx-xxx-xxx-xxx'})
    class Meta:
        model = Supplier
